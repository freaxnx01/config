...
then follow it exactly. This command holds no query logic of its own — `/gh:issues`
and `/fj:issues` remain the single source of truth.
...
- **github** → read and follow `~/.claude/commands/gh/issues.md` (i.e. run `/gh:issues`).
- **forgejo** → read and follow `~/.claude/commands/fj/issues.md` (i.e. run `/fj:issues`).
